    ...
    case weather of
        Just w -> do
            ...
            case topArtists of
                Just v -> do
                    let myJSON = show v
                    let myByteString = C.pack myJSON
                    let myLazyByteString = BL.fromStrict myByteString
                    case decode myLazyByteString of
                        Just myJSONObject -> do
                            let textValue = text (head (parts (head (candidates myJSONObject))))
                            print textValue
