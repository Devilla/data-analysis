...
main = do
    ...
    case weather of
        Just w -> do
            ...
            case topArtists of
                Just v -> do
                    let text = case fromJSON v of
                                Aeson.Success (MyJSON candidates) -> do
                                    let textValue = text (head (parts (head candidates)))
                                    putStrLn ("Top 10 artists and their hometown's famous food: " ++ textValue)
                                Aeson.Error _ -> putStrLn "Error parsing JSON"
                    putStrLn $ "Top 10 artists and their hometown's famous food: " ++ text
